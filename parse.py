...
class Ad2cpReader:
    def __init__(self, f: TextIO, data_record_format_type: BurstAverageDataRecordVersion):
        self.packets = []
        while True:
            try:
...
class Ad2cpDataPacket:
    def __init__(self, f: TextIO, burst_average_data_record_version: BurstAverageDataRecordVersion):
    ...
    def _read_header(self, f: TextIO):
        ...
        print("header checksum     ", self.checksum(
            raw_header[: -2]), self.header_checksum)
    ...
    def _read_data_record(self, f: TextIO):
        ...
        print("data record checksum", self.checksum(
            raw_data_record), self.data_record_checksum, end="\n\n")
        assert self.checksum(
    ...
    HEADER_FORMAT: List[type_field] = [
        ("sync", 1, False),
        ("header_size_bytes", 1, False),
        ("id", 1, False),
        ("family", 1, False),
        ("data_size_bytes", 2, False),
        ("data_record_checksum", 2, False),
        ("header_checksum", 2, False),
    ]
    STRING_DATA_RECORD_FORMAT: List[type_field] = [
        ("id", 1, None),
        ("string_data", lambda self: self.data_size_bytes - 1, None)
    ]
